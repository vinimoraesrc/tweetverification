...
def _convert_format(pairs, texts):
    ...
    converted = pd.DataFrame(
        {'tweets_a': texts_a, 'tweets_b': texts_b, 'labels': labels})
    return converted
...
def main():
    ...
    train_authors, train_tweet_ids = _create_set(
        tweets, "same_authors_train", index_name, 10, None, None, 1400, 15, 15,
        None, None, texts)

    val_authors, val_tweet_ids = _create_set(
        tweets, "same_authors_val", index_name, 10, train_authors, "same", 300,
        15, 15, train_tweet_ids, "disjoint", texts)
